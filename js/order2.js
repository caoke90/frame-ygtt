...
var errorUrl = PurchaseAppConfig.errorUrl;
...
function addLimitInfo(){
	...
	$("#dongDiv .couponNoSelect").add(".#dongDiv .couponSelect").find(".tip").each(function(){
        var $this = $(this),
            parent = !!$this.parents(".couponNoSelect").length ? $this.parents(".couponNoSelect") : $this.parents(".couponSelect"),
            dialog = parent.find(".tip-dialog");
...
function queryLipinkas(result) {
    $("#lipinkasArea").html("<span class='waitInfo' id='waitInfo'>正在获取礼品卡，请等待！</span>");
	var result=result||PurchaseAppConfig.data
            if (result != null) {
                if (result.error != null && result.error != "") {
                    if (result.error == "NoCart") {
                        goCart();
            ...
            $("#lipinkasArea").html(TrimPath.processDOMTemplate("lipinkasData", param));
...
function isNeedPaymentPassword(result) {
...
function goCart() {
    window.location.href = PurchaseAppConfig.cartUrl + Math.random();
}
function goLogin() {
    window.location.href = PurchaseAppConfig.OrderLoginUrl;
}
function goError() {
    window.location.href = PurchaseAppConfig.errorUrl + Math.random();
}
        ...
        success:function (data) {
            if (data != null) {
                ...
                if (data.flag == true && data.consigneeInfo != 'undefined' && data.consigneeInfo != null) {
                	var consignee=data.consigneeInfo;
                	if(consignee.address != null){
                		consignee.address = consignee.address.replace(/<\/?[^>]*>/g,'').replace(/&nbsp;/ig,'');
                    	data.consigneeInfo = consignee;
                	}
...
function check_area() {
    ...
    if (!$("#div_Towns").is(":hidden") && townValue == '-22') {//有四级地址,但是没选择
        $("#area_desc_span").hide();
        showAlert('请选择您所在的街道/乡镇！', g('checkAreaDiv'), 'area_check');
        return false;
    }
    if (proValue == '-22' || cityValue == '-22' || countryValue == '-22'
        ...
        $("#area_desc_span").hide();
        showAlert('地区信息不完整！', g('checkAreaDiv'), 'area_check');
        return false;
    }
    return true;
}
...
function check_address() {
    ...
    if (!is_forbid(g('consignee_address').value)) {
...
function check_phone() {
    ...
    var result = phoneNum.match(/^[0-9\-－]*$/ig);
    if(result==null){
...
function savePart_consignee(obj) {
    clearWaitInfo();
    if (check_all()) {
        data = "ocName=" + encodeURIComponent(encodeURIComponent(g('consignee_addressName').value)) +
            "&ocAddress=" + encodeURIComponent(encodeURIComponent(g('consigneeShow_addressName').innerHTML + g('consignee_address').value)) +
            "&ocMessage=" + g('consignee_message').value + "&ocPhone=" + g('consignee_phone').value +
            "&ocEmail=" + g('consignee_email').value + "&ocPost=" + g('consignee_postcode').value +
            "&provinceId=" + g('consignee_province').value + "&cityId=" + g('consignee_city').value +
            "&countyId=" + g('consignee_county').value + "&easyBuy=" + isEasyBuy + "&addressId=" + selAddressId +
            "&townId=" + g('consignee_towns').value;
...
function getInstallmentInfo(){
	...
	jQuery.ajax({
        ...
        dataType:"json",
        async:false,
        url:PurchaseAppConfig.OrderDomain + "/getInstallmentInfo.action?rt=" + Math.random(),
        success:function (data) {
			if (data != null && data.giir != null && data.giir.flag == true) {
				//如果分期信息为空则不显示
				if(data.giir.installmentInfoList == null || data.giir.installmentInfoList.length<=0){
					g('payRemark_8').style.display = 'none';
				}else{
					installmentInfo = data.giir;
					//先清空再展现
					$("#banks").empty();
					$("#banks").append(TrimPath.processDOMTemplate("installment_banks", installmentInfo));
					g('payRemark_8').style.display = '';
					changeBank(installmentInfo.selectedBankId, false);
...
function changeBank(bankId, flag){
    ...
    if(flag){
    	...
    	changePeriod(bankId, period, getRate(bankId, period));
    }
}
//修改分期数
function changePeriod(bankId, period, rate){
	var totalMoney = $("#needPayPrice").val();
...
function changePayType(payType) {
    ...
    if (payType == 1) {
        if($("#payRemark_1").length>0){
            g('payRemark_1').style.display = '';
        }
    } else {
        if($("#payRemark_1").length>0){
            g('payRemark_1').style.display = 'none';
        }
    }
    if (payType == 4) {
        ...
        success:function (data) {
            if (data != null) {
                ...
                if(data.sts != null){
                	...
                	//如果选择其他三种配送方式则重置预约时间下拉框为[请选择时间]
                	$("#CODTime1,#CODTime2,#CODTime3").click(function(){
                		$("#payType_CodDate option:first").attr("selected",true);
                	});
                	//中小件预约配送.如果选择预约时间时,未选择预约配送配送方式,则将该配送方式置为选中状态
                	$("#payType_CodDate").change(function(){
                		var value =$("#payType_CodDate").find("option:selected").val();
                		if(value != -1){
                			$("#CODTime4").attr("checked",true);
                		}
                	});
                } 
            }
...
function savePayTypeAndShipType(obj) {
    ...
    /**
     * *****************************************************************************
     * ADDED BY liuxiaori for reserve ship
     * @date 2012-11-09
     * 验证预约配送时间
     * 
     * 此处代码与buildPSData有重复
    ...
    if(codId == 4){
        if(g('payType_CodDate')!=null){
            if(g('payType_CodDate').value == -1){
                showAlert('请选择中小件商品预约送货时间！',pNode,'payType_selectPayType');
                return;
            }
        }
    }
	...
	if(g('payType_InstallDate')){
		if(g('payType_InstallDate').value ==-1){
			showAlert('请选择大家商品安装时间！',pNode,'payType_selectPayType');
			return;
		}
	}
    if (pid == 19) {
    ...
    //分期付款
    if(pid == 8){
    	var bankId = $("input[name='ins_banks'][checked]");
    	var period = $("input[name='ins_periods'][checked]");
    	if(bankId == null || period == null){
    		showAlert('请选择分期银行！',pNode,'payType_selectPayType');
            return;
    	}
    	if(bankId.val() == 'undefined' || period.val() == 'undefined'){
    		showAlert('请选择分期付款分期数！',pNode,'payType_selectPayType');
            return;
    	}
    }
...
function buildPSData(pid) {
    ...
    if (pid == 1 || pid == 2 || pid == 4 || pid == 5 || pid == 8) {
		...
		if(g('payType_InstallDate')!=null){
			data=data+"&payType_InstallDate="+g('payType_InstallDate').value;
		}
        /****************************************************************************
         * ADDED BY liuxiaori 
         * @date 2012-10-24
         * 如果为预约配送,把页面中id为payType_CodDate元素值,转存到cdate参数
         */
        if(codId==4){
            var cdate = "";
            if(g('payType_CodDate')!=null){
                cdate = g('payType_CodDate').value;
                data=data+"&cdate="+cdate;
            }
        }
        /***************************************************************************/
        //增加分期付款参数信息
        if(pid == 8){
        	data=data+"&selectedBankId=" + $("input[name='ins_banks'][checked]").val()
        			+ "&selectedPeriod=" + $("input[name='ins_periods'][checked]").val();
        }
    } else if (pid == 3) {//京东自提
        var pickSiteId = '';
...
//	   }
        ...
        var res = true;
        if (!check_Ivc_TitName()) {
            res = false;
        }
        if (!check_NsrCode()) {
            res = false;
        }
        if (!check_InvoiceAddress()) {
            res = false;
        }
        if (!check_InvoicePhone()) {
            res = false;
        }
        if (!check_InvoiceBank()) {
            res = false;
        }
        if (!check_InvoiceBankCode()) {
            res = false;
        }
        return res;
    }
...
function DoOnInvoiceTypeChange() {
    ...
    if (panelremark != null) {
        ...
        if (ivoitype == 2) {
            panelremark.innerHTML = "<b>提示：</b>尊敬的客户您好，如果您需要开具增值税发票，建议您联系<a href='https://passport.jd.com/new/registcompany.aspx' style='color: rgb(0, 112, 215);' target='_blank'>企业客户</a>代表，将有专人为您处理！";
        }
    }
}
...
function check_invoice_unit() {
    ...
    if (isEmpty('invoice_Unit_TitName')) {
        showAlert('单位名称不能为空！', pNode, 'Invoice_UnitEmpty');
        return false;
    }
    ...
    if (!is_forbid(g('invoice_Unit_TitName').value)) {
        showAlert(ffAlertTxt, pNode, 'Invoice_UnitFf');
        return false;
    }
    return true;
}
...
function check_Ivc_TitName() {
    ...
    if (isEmpty('invoice_Ivc_TitName')) {
        showAlert('单位名称不能为空！', pNode, 'Invoice_TitNameEmpty');
        return false;
    }
    ...
    if (!is_forbid2(g('invoice_Ivc_TitName').value)) {
        showAlert(ffAlertTxt, pNode, 'Invoice_TitNameFf');
        return false;
    }
    return true;
}
...
function check_NsrCode() {
    ...
    if (isEmpty('invoice_Ivc_NsrCode')) {
        showAlert('纳税人识别号不能为空！', pNode, 'Invoice_NsrCodeEmpty');
        return false;
    }
    ...
    if (!is_forbid(g('invoice_Ivc_NsrCode').value)) {
        showAlert(ffAlertTxt, pNode, 'Invoice_NsrCodeFf');
        return false;
    }
    return true;
}
...
function check_InvoiceAddress() {
    ...
    if (isEmpty('invoice_Ivc_Address')) {
        showAlert('注册地址不能为空！', pNode, 'Invoice_addressEmpty');
        return false;
    }
    ...
    if (!is_forbid2(g('invoice_Ivc_Address').value)) {
        showAlert(ffAlertTxt, pNode, 'Invoice_addressFf');
        return false;
    }
    return true;
}
...
function check_InvoicePhone() {
    ...
    if (isEmpty('invoice_Ivc_Phone')) {
        showAlert('注册电话不能为空！', pNode, 'Invoice_PhoneEmpty');
        return false;
    }
    ...
    if (!is_forbid2(g('invoice_Ivc_Phone').value)) {
        showAlert(ffAlertTxt, pNode, 'Invoice_PhoneFf');
        return false;
    }
    return true;
}
...
function check_InvoiceBank() {
    ...
    if (isEmpty('invoice__Ivc_Bank')) {
        showAlert('开户银行不能为空！', pNode, 'Invoice_bankEmpty');
        return false;
    }
    ...
    if (!is_forbid2(g('invoice__Ivc_Bank').value)) {
        showAlert(ffAlertTxt, pNode, 'Invoice_bankFf');
        return false;
    }
    return true;
}
...
function check_InvoiceBankCode() {
    ...
    if (isEmpty('invoice_Ivc_BankCode')) {
        showAlert('银行帐户不能为空！', pNode, 'Invoice_bankCodeEmpty');
        return false;
    }
    if (!is_forbid2(g('invoice_Ivc_BankCode').value)) {
        showAlert(ffAlertTxt, pNode, 'Invoice_bankCodeFf');
        return false;
    }
    return true;
}
        ...
        jQuery.ajax({
            ...
            success:function (result) {
                ...
                if (result.sor != null && result.sor.flag) {
                    clearWaitInfo();
                    $("#submitWaitInfo").html("<span><font color='#01B703'>订单提交成功，正在转向提交成功页！</font></span>");
                    //提交订单成功，但写入缓存时出错
                    if ("SubmitSuccess_CacheError" == result.sor.resultCode) {
                    	writeCn();
                        successUrl = "'http://cart.jd.com/order/successOrder/successDefault.action?orderid=" + result.sor.orderId+"'";
                    	window.setTimeout("window.location.href="+successUrl,300);
                        return;
                    }
                    writeCn();
                    successUrl = "'http://cart.jd.com/order/successOrder/getSuccessOrder.action?orderid=" + result.sor.orderId+"'";
                    window.setTimeout("window.location.href="+successUrl,300);
			return ;
                } else {
                    clearWaitInfo();
...
function getPromiseInfo(param, node) {
    $.getJSON(
        //"http://jd2008.jd.com/purchaseservice/ajaxServer/ForPromiseDate.aspx?action=GetOrderPrmoise&callback=?",
        //"http://jd2008.jd.com/purchaseservice/ajaxServer/ForPromiseDate.aspx?action=GetOrderPrmoise&callback=?",
...
function isSupInStockFirstShip(result) {
...
function getCheckCode(obj) {
    cleanCheckCode();
    obj.src = "http://cart.jd.com/order/getCheckCode.action?id=" + Math.random();
    $('#checkcodeTxt').val("");
}
 ...
 * 是否显示订单备注
 */
function showOrderRemark(result) {
	var result=result||PurchaseAppConfig.data
...
function changeCodDate(codDate){
	//获取支付方式
	var payTypeRadio = gn("input","IdPaymentType");
	var pid = 0;
	for(i=0;i<payTypeRadio.length;i++){
	...
	if(g('payType_InstallDate')!=null){
		if(codDate==-1){
			$('#payType_InstallDate').html('<option value="-1">请选择日期</option>');
		}else{
			var insallDateUrl = PurchaseAppConfig.OrderDomain+"/getInstallDates.action?rt="+Math.random();	
			jQuery.ajax({
				type : "POST",
				dataType : "json",
				url : insallDateUrl,
				data : {installCodeDate:codDate,pid:pid},
				success : function(data) {
					if(data != null){
						$('#payType_InstallDate').html(data.installDateOptions);
					}			   
				}
			});
		}
